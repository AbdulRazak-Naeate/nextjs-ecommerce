  ...
  const handleSubmit = async (values: z.infer<typeof ProductFormSchema>) => {
    try {
      ...
      const currentValues = form.getValues();
        ...
        const name = (currentValues.name ?? values.name)?.toString().trim();
        const description = (currentValues.description ?? values.description)?.toString().trim();
        const variantName = (currentValues.variantName ?? values.variantName)?.toString().trim();
        const variantDescription = (currentValues.variantDescription ?? values.variantDescription)?.toString().trim();
        const variantImage = (currentValues.variantImage && currentValues.variantImage[0]
          ? currentValues.variantImage[0].url
          : null) || (values.variantImage && values.variantImage[0] ? values.variantImage[0].url : null);
        const images = (  currentValues.images ?? values.images);
        const categoryId = (currentValues.categoryId ?? values.categoryId)?.toString().trim();
          const subCategoryId = (currentValues.subCategoryId ?? values.subCategoryId)?.toString().trim();
          const offerTagId = (currentValues.offerTagId ?? values.offerTagId)?.toString().trim();
          const isSale = (currentValues.isSale ?? values.isSale) ?? false;
          const saleEndDate = (currentValues.saleEndDate ?? values.saleEndDate)?.toString().trim();
          const brand = (currentValues.brand ?? values.brand)?.toString().trim();
          const sku = (currentValues.sku ?? values.sku)?.toString().trim();
          const weight = Number((currentValues.weight ?? values.weight)?.toString().trim());
          const colors = (currentValues.colors ?? values.colors);
          const sizes = (currentValues.sizes ?? values.sizes);
          const product_specs = (currentValues.product_specs ?? values.product_specs);
          const variant_specs = (currentValues.variant_specs ?? values.variant_specs);
          const keywords = (currentValues.keywords ?? values.keywords);
          const questions = (currentValues.questions ?? values.questions);
          const shippingFeeMethod = (currentValues.shippingFeeMethod ?? values.shippingFeeMethod);
          const freeShippingForAllCountries = (currentValues.freeShippingForAllCountries ?? values.freeShippingForAllCountries);
          const freeShippingCountriesIds = (currentValues.freeShippingCountriesIds ?? values.freeShippingCountriesIds);
      ...
      const response = await upsertProduct(
        {
          ...
          freeShippingCountriesIds: freeShippingCountriesIds || [],
          createdAt: new Date(),
          updatedAt: new Date(),
        },
        storeUrl
